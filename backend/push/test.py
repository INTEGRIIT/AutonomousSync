...
async def upload_file(
    ...
    user_id: str = Form(...),
    snapshot_id: str = Form(None),  # ✅ REQUIRED
    ...
    try:
        ...
        # ✅ STORE FILE UNDER SAME SNAPSHOT
        insert_snapshot({
            ...
            "url": result["url"],
            "ts": time.time(),
            "kind": "file",  # 🔥 distinguish from system snapshot
        })
...
def get_events(device_uid: str, limit: int = 50):
    try:
        ...
        # 🔥 JOIN FILES TO EVENTS
        snapshots_collection = events_collection.database["snapshots"]
